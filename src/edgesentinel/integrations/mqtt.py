...
Forwards events published on an :class:`~edgesentinel.core.events.EventBus` to
...
:mod:`edgesentinel.integrations.http`, there is no MQTT client in the standard
library -- production use requires the ``paho-mqtt`` package (``pip install
edgesentinel[mqtt]``). This module itself has no import-time dependency on it,
...
logger = logging.getLogger("edgesentinel.integrations.mqtt")
...
_DEFAULT_TOPIC: TopicBuilder = _default_topic("edgesentinel")
...
def _build_default_client() -> MqttClient:
    try:
        ...
        raise ImportError(
            "edgesentinel.integrations.mqtt needs a real MQTT client when none "
            "is supplied via the `client` argument. Install the optional "
            "dependency with: pip install edgesentinel[mqtt]"
        ) from exc
...
class MqttPublisher:
    ...
    :class:`~edgesentinel.persistence.database.Database` uses for ``sqlite3``,
