"""The EdgeSentinel runtime: the top-level object applications construct.

``EdgeSentinel`` composes the lifecycle state machine, the event bus, and local
...
from edgesentinel.core.events import EventBus
from edgesentinel.core.exceptions import RuntimeAlreadyStartedError, RuntimeNotStartedError
from edgesentinel.core.lifecycle import LifecycleManager, StateChangeHandler
from edgesentinel.core.state import RuntimeState, StateMachine
from edgesentinel.diagnostics.incidents import IncidentTracker
from edgesentinel.diagnostics.timeline import EventLog
from edgesentinel.durability.journal import IntentJournal
...
from edgesentinel.metrics.checks import read_hardware_status
from edgesentinel.metrics.monitor import MetricsCheck, MetricsMonitor, Mitigation
...
from edgesentinel.process.supervisor import Supervisor, TaskFunc
from edgesentinel.process.watchdog import Watchdog
from edgesentinel.resilience.backoff import BackoffAlgorithm
from edgesentinel.resilience.circuit_breaker import CircuitBreaker
from edgesentinel.resilience.policy import build_reliable_decorator
from edgesentinel.resilience.retry import RetryPolicy
from edgesentinel.storage.monitor import Cleanup, StorageMonitor

logger = logging.getLogger("edgesentinel.runtime")
...
class EdgeSentinel:
    """Entry point for the edgesentinel reliability runtime.
    ...
    def incidents(self) -> IncidentTracker:
        ...
        history instead, see
        :func:`~edgesentinel.diagnostics.incidents.build_incidents`.
        """
        ...
        *,
        ...
        Args:
            ...
            backoff, initial_delay, max_delay, jitter, retry_on: Shorthand
                for building a :class:`~edgesentinel.resilience.retry.RetryPolicy`;
            ...
            circuit_breaker: ``True`` creates a private breaker with default
                ...
                :class:`~edgesentinel.resilience.circuit_breaker.CircuitBreaker`
        ...
        failed" -- see :mod:`edgesentinel.resilience.policy` for the rationale.
        """
    ...
    def durable(
        ...
        *calling* the decorated function requires the runtime's database
        ...
        :class:`~edgesentinel.core.exceptions.RuntimeNotStartedError` if the
        runtime hasn't started yet.

        Args:
            operation: Stable name identifying this operation in the
                journal and in replay. Must be unique per ``EdgeSentinel``
            ...
            max_attempts: Total attempts, counted across the original call
                ...
                :class:`~edgesentinel.core.exceptions.DurableOperationExhaustedError`
    ...
    def watch_network(
        ...
        *,
        ...
        """Create a :class:`~edgesentinel.network.monitor.NetworkMonitor` wired
        to this runtime.
        ...
        Args:
            checks: See :class:`~edgesentinel.network.monitor.NetworkMonitor`.
    ...
    def supervise(
        ...
        *,
        ...
        """Create a :class:`~edgesentinel.process.supervisor.Supervisor` wired
        to this runtime.
        ...
        Args:
            ...
            max_delay, jitter: See
                :class:`~edgesentinel.process.supervisor.Supervisor`.
        """
    ...
    def watchdog(self) -> Watchdog:
        """A :class:`~edgesentinel.process.watchdog.Watchdog` shared by this
    ...
    def watch_storage(
        ...
        *,
        ...
        """Create a :class:`~edgesentinel.storage.monitor.StorageMonitor` wired
        to this runtime.
        ...
        Args:
            path, low_water_bytes, low_water_inodes, cleanup, interval: See
                :class:`~edgesentinel.storage.monitor.StorageMonitor`.
        """
    ...
    def watch_hardware(
        ...
        *,
        ...
        """Create a :class:`~edgesentinel.metrics.monitor.MetricsMonitor` wired
        to this runtime.
        ...
        Args:
            ...
            interval, metrics_check: See
                :class:`~edgesentinel.metrics.monitor.MetricsMonitor`.
        """
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            self._started = True
            self._stopped = False
        logger.info("edgesentinel runtime %r started", self._name)
    ...
    async def stop(self) -> None:
        ...
        async with self._lock:
            ...
            await self._database.close()
            self._stopped = True
        logger.info("edgesentinel runtime %r stopped", self._name)
    ...
    async def __aenter__(self) -> EdgeSentinel:
        await self.start()
        return self
