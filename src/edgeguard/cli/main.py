"""``edgeguard`` CLI entry point.

A synchronous ``argparse`` front end over the async diagnostics APIs
(:class:`~edgeguard.persistence.database.Database`,
:class:`~edgeguard.diagnostics.timeline.EventLog`,
:func:`~edgeguard.diagnostics.incidents.build_incidents`) -- the CLI wraps a
...
from edgeguard.core.events import Severity
from edgeguard.diagnostics.incidents import build_incidents
from edgeguard.diagnostics.report import format_incidents, format_timeline, summarize_incidents
...
def build_parser() -> argparse.ArgumentParser:
    """Build the top-level ``edgeguard`` argument parser."""
    parser = argparse.ArgumentParser(
        prog="edgeguard",
        description=(
            "Inspect an edgeguard runtime's local state. Reads the runtime's "
    ...
    parser.add_argument(
        "--name",
        required=True,
        help="Runtime name -- the same value passed to EdgeGuard(name, ...).",
    )
    parser.add_argument(
        "--data-dir",
        default="./data",
