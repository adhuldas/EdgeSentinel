...
edgeguard's Phase 4 subsystems (network, process, storage) don't report
their health to applications directly -- they escalate through the
lifecycle state machine (see :mod:`edgeguard.core.state`), so a
...
from edgeguard.core.events import Event, EventBus, EventHandler
...
class IncidentTracker:
    ...
    Args:
        max_history: Number of closed incidents to retain in memory, oldest
            ...
            the durable record from :class:`~edgeguard.diagnostics.timeline.EventLog`.
...
def build_incidents(events: Iterable[Event]) -> list[Incident]:
    ...
    :meth:`~edgeguard.diagnostics.timeline.EventLog.query`. Used by the
