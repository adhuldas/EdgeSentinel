...
Forwards events published on an :class:`~edgeguard.core.events.EventBus` as
...
:class:`~edgeguard.persistence.database.Database` uses for ``sqlite3``, so
...
logger = logging.getLogger("edgeguard.integrations.http")
...
#: the same way :mod:`edgeguard.network.monitor` checks are.
...
class HttpEventPublisher:
    ...
    Example:
        >>> publisher = HttpEventPublisher(url="https://example.com/hooks/edgeguard")
