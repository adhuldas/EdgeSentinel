"""Exception hierarchy for edgeguard.

Every exception raised directly by edgeguard inherits from
:class:`EdgeGuardError`, so applications can catch library failures with a
...
class EdgeGuardError(Exception):
    """Base class for all exceptions raised by edgeguard."""


class InvalidStateTransitionError(EdgeGuardError):
    """Raised when a lifecycle state transition is not permitted.

    edgeguard's runtime states form a fixed graph (see
    :mod:`edgeguard.core.state`). Attempting to move to a state that isn't
...
class RuntimeAlreadyStartedError(EdgeGuardError):
    """Raised when ``start()`` is called on a runtime that is already running."""


class RuntimeNotStartedError(EdgeGuardError):
    """Raised when an operation requires a running runtime that was never started."""


class InvalidDurablePayloadError(EdgeGuardError, TypeError):
...
class DurableOperationExhaustedError(EdgeGuardError):
