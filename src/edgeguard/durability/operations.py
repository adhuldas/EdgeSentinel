...
``EdgeGuard.durable()`` is the primary developer-facing API; it delegates to
...
from edgeguard.durability.journal import Intent, IntentJournal

logger = logging.getLogger("edgeguard.durability")
...
def build_durable_decorator(
    *,
    ...
    """Build the decorator returned by ``EdgeGuard.durable(operation, ...)``.

    See :meth:`edgeguard.core.runtime.EdgeGuard.durable` for parameter
...
async def replay_pending(
    ...
    *,
    ...
    Called once during ``EdgeGuard.start()``, before the runtime is
