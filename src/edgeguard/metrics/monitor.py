...
degrade in ways :class:`~edgeguard.network.monitor.NetworkMonitor` and
:class:`~edgeguard.storage.monitor.StorageMonitor` can't see -- the network
...
Metrics are read via an injectable check (see :mod:`edgeguard.metrics.checks`
...
from edgeguard.metrics.checks import HardwareStatus, read_hardware_status

logger = logging.getLogger("edgeguard.metrics")
